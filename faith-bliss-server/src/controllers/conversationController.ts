...
export const getConversationIdByProfileId = async (
  ...
  try {
    ...
    const conversationId = queryResult.docs[0].id;
...
export const getConversationMessages = async (
  ...
  try {
    ...
    if (
      ...
      !matchData.users.includes(currentUserId)
    ) {
      return res
        .status(403)
    ...
    const userMap: Record<
      string,
      { id: string; name: string; profilePhoto1?: string }
    > = {};
    ...
    const messages: Message[] = snapshot.docs.map((doc) => {
      ...
      return {
        ...
        sender: userMap[data.senderId] || {
          id: data.senderId,
          name: "Unknown",
        },
        receiver: userMap[data.receiverId] || {
          id: data.receiverId,
          name: "Unknown",
        },
      } as Message;
