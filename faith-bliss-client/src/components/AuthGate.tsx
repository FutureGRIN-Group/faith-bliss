...
export const AuthGate: React.FC = () => {
  ...
  const obStatus = user?.onboardingCompleted
    ? "Complete"
    : user
    ? "Pending"
    : "N/A";
  console.log(
    `E. AUTH_GATE CHECK: Path=${path}, Loading=${isLoading}, Auth=${isAuthenticated}, Onboarding=${obStatus}`
  );
  ...
  if (!isAuthenticated) {
    // --- LOG (F) ---
    console.log(
      "F. AUTH_GATE REDIRECT: User is NOT authenticated. Redirecting to /login."
    );
  ...
  if (user && !user.onboardingCompleted) {
    ...
    if (!path.startsWith("/onboarding")) {
      // --- LOG (G) ---
      console.log(
        "G. AUTH_GATE REDIRECT: Onboarding is PENDING. Redirecting to /onboarding."
      );
      return <Navigate to="/onboarding" replace />;
    }
    // --- LOG (H) ---
    console.log(
      "H. AUTH_GATE ACCESS: Onboarding is PENDING, allowing access to /onboarding."
    );
    return <Outlet />;
  }
  ...
  if (user && user.onboardingCompleted) {
    ...
    if (path.startsWith("/onboarding")) {
      // --- LOG (I) ---
      console.log(
        "I. AUTH_GATE REDIRECT: Onboarding is COMPLETE. Redirecting AWAY from /onboarding to /dashboard."
      );
    ...
    // --- LOG (J) ---
    console.log(
      "J. AUTH_GATE ACCESS: Onboarding is COMPLETE, allowing access."
    );
    return <Outlet />;
  }
...
export const PublicOnlyRoute: React.FC = () => {
  ...
  if (isAuthenticated) {
    ...
    const targetPath =
      user && !user.onboardingCompleted ? "/onboarding" : "/dashboard";
    // --- LOG (K) ---
    console.log(
      `K. PUBLIC_GATE REDIRECT: Logged in. Redirecting from public route to ${targetPath}.`
    );
