...
const fetchUserDataFromFirestore = async (
  ...
  if (!docSnap.exists()) {
    console.log(
      `❌ Firestore: Profile document for ${fbUser.uid} not found. Returning minimal data.`
    );
    return null;
  }
...
export function useAuth() {
  ...
  const completeOnboarding = useCallback(
    ...
    async (onboardingData: OnboardingData) => {
      ...
      try {
        console.log(
          "G. Firestore: Attempting to complete onboarding for user:",
          fbUser.uid
        );
        const userDocRef = doc(db, "users", fbUser.uid);
  ...
  useEffect(() => {
    if (!isLoading && user) {
      ...
      const isTransientRoute = ["/", "/login", "/register"].includes(
        location.pathname
      );
      ...
      console.log(
        `✅ Navigation Stable: Allowed access to ${location.pathname}.`
      );
    }
  ...
  const getUserProfileById = useCallback(
    async (userId: string): Promise<User | null> => {
      ...
      try {
        ...
        console.log(
          `✅ Firestore: Successfully fetched user profile for UID: ${userId}`
        );
        return profile;
      } catch (error: any) {
        console.error(
